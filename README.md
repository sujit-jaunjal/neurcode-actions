...
| How do I start as an OSS maintainer? | Install the RC5 workflow on one repo or one PR and score whether the summary is actionable, obvious, or noisy. |
...
| Is RC5 stable? | No. `v0.3.0-rc.5` is the current rehearsal ref. `v0.2.4` remains the existing stable public release. |
...
`sujit-jaunjal/neurcode-actions@v0.2.4` remains available and untouched as the existing stable pilot surface. `v0.3.0-rc.5` is the current rehearsal ref for runtime admission bridge behavior after live Airflow fork rehearsal.
